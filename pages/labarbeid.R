...
labarbeid_server <- function(id, login_import) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$proc_text <- renderText("Særskilt malaisefeller fanger store mengder insekter, og en manuell sortering og identifisering av prøvene i et så stort program som dette ville for kostsamt og bruke for lang tid. Manuell identifisering stiller også store krav på taksonomisk kompetanse, men er mulig å gjennomføre for enkelte grupper, som for eksempel biller eller blomsterfluer, for kvalitetssikring av metodikken. 
